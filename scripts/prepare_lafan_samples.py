...
Clips are trimmed to at least 60 s each while keeping the static bundle reasonable.
...
# LAFAN1 clips are 30 fps; keep at least 60 s per bundled sample.
MIN_FRAMES = 1800  # 60 s @ 30 fps

# output name -> (source bvh in zip, start frame, frame count)
SAMPLES = {
    "walk.bvh": ("walk1_subject1.bvh", 0, MIN_FRAMES),
    "run.bvh": ("run1_subject5.bvh", 0, MIN_FRAMES),
    "dance.bvh": ("dance1_subject1.bvh", 0, MIN_FRAMES),
    "fall_getup.bvh": ("fallAndGetUp3_subject1.bvh", 150, MIN_FRAMES),
    "jumps.bvh": ("jumps1_subject1.bvh", 0, MIN_FRAMES),
}
